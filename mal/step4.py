...
def EVAL(ast: Any, env: Env) -> Any:
    ...
    if isinstance(first, MalSymbol):
        ...
        if first.value == 'fn*':
            # (fn* params body...) - returns a function
            params = ast.items[1]
            body_exprs = ast.items[2:]

            # Create a closure
            def fn_callable(*args):
                # Create new environment with outer = current env
                fn_env = Env(outer=env)

                # Handle variadic parameters with & more
                param_items = params.items if isinstance(params, (MalList, MalVector)) else []
                ...
                if variadic_index is not None:
                    ...
                    if isinstance(params, MalList):
                        param_symbols = params.items
                        for i, param in enumerate(param_symbols):
                            if i < len(args):
                                fn_env.set(param.value, args[i])
                            else:
                                fn_env.set(param.value, NIL)
                    elif isinstance(params, MalVector):
                        param_symbols = params.items
                        for i, param in enumerate(param_symbols):
                            if i < len(args):
                                fn_env.set(param.value, args[i])
                ...
                # Evaluate body expressions and return last result
                result = NIL
                for body_expr in body_exprs:
                    result = EVAL(body_expr, fn_env)
                return result
    ...
    if isinstance(evaluated, MalList) and len(evaluated.items) > 0:
        ...
        # If func is a Python callable (from fn*), call it
        if callable(func):
            return func(*args)
