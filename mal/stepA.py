#!/usr/bin/env python3
"""
Mal Step A: Mal Self-Host

Step A completes the Mal implementation:
- All features from Steps 0-9
- Capable of evaluating Mal code that implements Mal
"""
...
class MalThrownException(Exception):
    """Exception thrown by the throw function"""
...
class MalMacro(MalType):
    """
    Macro type - like a function but returns code to be evaluated.
    Wraps a MalFunction with macro behavior.
    """
    def __init__(self, func):
        self.func = func  # The underlying MalFunction

    def __str__(self):
        return "#<macro>"

    def __eq__(self, other):
        return isinstance(other, MalMacro)
...
def quasiquote_loop(acc: Any, elt: Any) -> Any:
    """Helper for quasiquote - process a single element"""
    # Check if elt is (splice-unquote form)
    if isinstance(elt, MalList) and len(elt.items) > 0:
        first = elt.items[0]
        if isinstance(first, MalSymbol) and first.value == 'splice-unquote':
            # (splice-unquote form) -> (concat form acc)
            if len(elt.items) == 2:
                return MalList([MalSymbol('concat'), elt.items[1], acc])
            else:
                raise Exception("splice-unquote: bad arguments")
    # Otherwise: (cons (quasiquote elt) acc)
    return MalList([MalSymbol('cons'), quasiquote(elt), acc])


def quasiquote_foldr(seq: List[Any]) -> Any:
    """Fold right for quasiquote list processing"""
    result = MalList([])
    for elt in reversed(seq):
        result = quasiquote_loop(result, elt)
    return result
...
def quasiquote(ast: Any) -> Any:
    ...
    Rules:
    - If ast is Map or Symbol: return (quote ast)
    - If ast is Vector: return (vec (quasiquote_foldr ast))
    - If ast is List and starts with unquote: return the unquoted value
    - If ast is List: return (quasiquote_foldr ast)
    - Otherwise: return ast as-is
    """
    if isinstance(ast, (MalHashMap, MalSymbol)):
        return MalList([MalSymbol('quote'), ast])
    elif isinstance(ast, MalVector):
        return MalList([MalSymbol('vec'), quasiquote_foldr(ast.items)])
    elif isinstance(ast, MalList):
        if len(ast.items) > 0:
            first = ast.items[0]
            if isinstance(first, MalSymbol) and first.value == 'unquote':
                if len(ast.items) == 2:
                    return ast.items[1]
                else:
                    raise Exception("unquote: bad arguments")
        return quasiquote_foldr(ast.items)
    else:
        return ast


def macroexpand(ast: Any, env: Env, once: bool = False) -> Any:
    """
    Expand macros in the AST.

    If once=True, expand only once.
    If once=False, expand repeatedly until no more macros.
    """
    while True:
        if not isinstance(ast, MalList) or len(ast.items) == 0:
            return ast

        first = ast.items[0]
        if not isinstance(first, MalSymbol):
            return ast

        # Look up the symbol in the environment
        try:
            func = env.get(first.value)
        except Exception:
            return ast

        if not isinstance(func, MalMacro):
            return ast

        # It's a macro! Expand it
        macro_func = func.func
        args = ast.items[1:]  # Unevaluated arguments
        ...
        # Bind parameters (not evaluating arguments for macros!)
        for i, param in enumerate(param_items):
            if i < len(args):
                macro_env.set(param.value, args[i])
            else:
        ...
        # Evaluate all but last expression, return last as expanded form
...
def EVAL(ast: Any, env: Env) -> Any:
    ...
    while True:
        ...
        if isinstance(first, MalSymbol):
            ...
            # Step 8: defmacro!
            if first.value == 'defmacro!':
                _, key, value_expr = ast.items
                value = EVAL(value_expr, env)
                if not isinstance(value, MalFunction):
                    raise Exception("defmacro! requires a function")
                macro = MalMacro(value)
            ...
            # Step 9: try* - TCO: tail position
            if first.value == 'try*':
                # (try* body (catch* exc handler)) or (try* body)
                if len(ast.items) == 2:
                    # With catch* handler
                    body = ast.items[1]
                    catch_clause = ast.items[2] if len(ast.items) > 2 else None
                elif len(ast.items) == 1:
                    # Just try* with no body, return nil
                    return NIL
                else:
                    # Try to parse: (try* body) or (try* body catch-clause)
                    body = ast.items[1] if len(ast.items) > 1 else None
                    catch_clause = ast.items[2] if len(ast.items) > 2 else None
                ...
                if catch_clause is None:
                    # No catch clause, just evaluate body
                    ast = body
                    continue
                else:
                    # Has catch clause
                    # Check if catch_clause is a list: (catch* exc handler)
                    if isinstance(catch_clause, MalList) and len(catch_clause.items) >= 3:
                        if isinstance(catch_clause.items[0], MalSymbol) and catch_clause.items[0].value == 'catch*':
                            exc_symbol = catch_clause.items[1]
                            handler = catch_clause.items[2]
                            if isinstance(exc_symbol, MalSymbol):
                                try:
                                    # Try to evaluate body
                                    result = EVAL(body, env)
                                    # If successful, return result (no TCO needed)
                                    return result
                                except MalThrownException as e:
                                    # Catch the exception and evaluate handler
                                    catch_env = Env(outer=env)
                                    catch_env.set(exc_symbol.value, e.value)
                                    ast = handler
                                    env = catch_env
                                    continue
                                    ...
                                    catch_env = Env(outer=env)
                                    catch_env.set(exc_symbol.value, MalString(str(e)))
                                    ast = handler
                                    env = catch_env
                                    continue
                            else:
                                raise Exception("catch* requires a symbol as exception name")
                        else:
                            # Not a catch* clause, might be a second body expression
                            # Treat as (try* body)
                            result = EVAL(body, env)
                            return result
                    else:
                        # Not a proper catch clause, just evaluate body
                        result = EVAL(body, env)
                        return result
            ...
            if first.value == 'quote':
                # (quote expr)
                if len(ast.items) != 2:
                    raise Exception("quote: bad arguments")
                return ast.items[1]

            # quasiquote - expand quasiquote notation
            if first.value == 'quasiquote':
                # (quasiquote expr)
                if len(ast.items) != 2:
                    raise Exception("quasiquote: bad arguments")
                # Expand quasiquote and then evaluate the result
            ...
            # Step 9: vector function
            if first.value == 'vector':
                # Create a vector from evaluated arguments
                return MalVector([EVAL(arg, env) for arg in ast.items[1:]])
            ...
            # Step 8: macro? function
            if first.value == 'macro?':
                # Check if something is a macro
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    return FALSE
                return MalBoolean(isinstance(args[0], MalMacro))

            # Step 8: vector? function
            if first.value == 'vector?':
                # Check if something is a vector
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    return FALSE
                return MalBoolean(isinstance(args[0], MalVector))

            # Step 8: symbol? function
            if first.value == 'symbol?':
                # Check if something is a symbol
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    return FALSE
                return MalBoolean(isinstance(args[0], MalSymbol))

            # Step 8: keyword? function
            if first.value == 'keyword?':
                # Check if something is a keyword
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    return FALSE
                return MalBoolean(isinstance(args[0], MalKeyword))

            # Step 8: map? function
            if first.value == 'map?':
                # Check if something is a hash map
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    return FALSE
                return MalBoolean(isinstance(args[0], MalHashMap))

            # Step 9: hash-map function
            if first.value == 'hash-map':
                # (hash-map key1 val1 key2 val2 ...) - create a hash map
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if len(args) % 2 != 0:
                    raise Exception("hash-map requires even number of arguments")
                hm_data = {}
                for i in range(0, len(args), 2):
                    ...
                    if isinstance(key, MalKeyword):
                        ...
                        dict_key = f"other:{key}"
                    hm_data[dict_key] = (key, value)
                return MalHashMap(hm_data)

            # Step 9: keys function
            if first.value == 'keys':
                # (keys hashmap) - get keys from hash map
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args or not isinstance(args[0], MalHashMap):
                    raise Exception("keys requires a hash map")
                hm = args[0]
                keys_list = []
                for dict_key, (orig_key, _) in hm.data.items():
                    keys_list.append(orig_key)
                return MalList(keys_list)

            # Step 9: vals function
            if first.value == 'vals':
                # (vals hashmap) - get values from hash map
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args or not isinstance(args[0], MalHashMap):
                    raise Exception("vals requires a hash map")
                hm = args[0]
                vals_list = []
                for dict_key, (_, value) in hm.data.items():
                    vals_list.append(value)
                return MalList(vals_list)

            # Step 9: assoc function
            if first.value == 'assoc':
                # (assoc hashmap key val ...) - add key-value pairs to hash map
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args or not isinstance(args[0], MalHashMap):
                    raise Exception("assoc requires a hash map as first argument")
                if len(args) % 2 != 1:
                    raise Exception("assoc requires key-value pairs after the hash map")
                hm = args[0]
                # Create a new hash map with updated data
                new_data = hm.data.copy()
                for i in range(1, len(args), 2):
                    ...
                    if isinstance(key, MalKeyword):
                        ...
                        dict_key = f"other:{key}"
                    new_data[dict_key] = (key, value)
                return MalHashMap(new_data)

            # Step 9: dissoc function
            if first.value == 'dissoc':
                # (dissoc hashmap key ...) - remove keys from hash map
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args or not isinstance(args[0], MalHashMap):
                    raise Exception("dissoc requires a hash map as first argument")
                hm = args[0]
                # Create a new hash map without the specified keys
                new_data = {}
                keys_to_remove = args[1:]
                for dict_key, (orig_key, value) in hm.data.items():
                    # Check if this key should be removed
                    should_remove = False
                    for remove_key in keys_to_remove:
                        if isinstance(remove_key, MalKeyword) and isinstance(orig_key, MalKeyword):
                            if remove_key.value == orig_key.value:
                                should_remove = True
                                break
                        elif isinstance(remove_key, MalString) and isinstance(orig_key, MalString):
                            if remove_key.value == orig_key.value:
                                should_remove = True
                                break
                        elif remove_key == orig_key:
                            should_remove = True
                            break
                    if not should_remove:
                        new_data[dict_key] = (orig_key, value)
                return MalHashMap(new_data)

            # Step 9: get function
            if first.value == 'get':
                # (get hashmap key default?) - get value from hash map
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    return NIL
                hm = args[0]
                if not isinstance(hm, MalHashMap):
                    return NIL
                if len(args) < 2:
                    return NIL
                key = args[1]
                default = args[2] if len(args) > 2 else NIL
                ...
                if dict_key in hm.data:
                    return hm.data[dict_key][1]
                else:
                    return default

            # Step 9: contains? function
            if first.value == 'contains?':
                # (contains? hashmap key) - check if key exists in hash map
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if len(args) < 2:
                    raise Exception("contains? requires 2 arguments")
                hm = args[0]
                if not isinstance(hm, MalHashMap):
                    return FALSE
                key = args[1]
                ...
                return MalBoolean(dict_key in hm.data)

            # Step 9: symbol function
            if first.value == 'symbol':
                # (symbol string) - create a symbol from a string
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    raise Exception("symbol requires an argument")
                if isinstance(args[0], MalString):
                    return MalSymbol(args[0].value)
                elif isinstance(args[0], MalSymbol):
                    return args[0]
                else:
                    raise Exception("symbol: argument must be a string")

            # Step 9: keyword function
            if first.value == 'keyword':
                # (keyword string) - create a keyword from a string
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    raise Exception("keyword requires an argument")
                if isinstance(args[0], MalString):
                    return MalKeyword(args[0].value)
                elif isinstance(args[0], MalKeyword):
                    return args[0]
                else:
                    raise Exception("keyword: argument must be a string")

            # Step 9: map function
            if first.value == 'map':
                # (map func seq) - apply function to each element of sequence
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if len(args) < 2:
                    raise Exception("map requires 2 arguments")

                func = args[0]
                seq = args[1]

                if not isinstance(seq, (MalList, MalVector)):
                    raise Exception("map: second argument must be a list or vector")
                    ...
                    if isinstance(func, MalFunction):
                        ...
                        if variadic_index is not None:
                            ...
                            for i, param in enumerate(regular_params):
                                if i < 1:  # Only one item
                                    fn_env.set(param.value, [item])
                                else:
                                    fn_env.set(param.value, NIL)

                            # Bind remaining arguments to rest_param as a list
                            if rest_param:
                                fn_env.set(rest_param.value, MalList([]))
                        else:
                        ...
                        if not body_items:
                            result_items.append(NIL)
                        else:
                            for body_expr in body_items[:-1]:
                                EVAL(body_expr, fn_env)
                            result_items.append(EVAL(body_items[-1], fn_env))
                    elif callable(func):
                        # Built-in function
                        result_items.append(func(item))
                    else:
                        raise Exception("map: first argument must be a function")

                return MalList(result_items)

            # Step 8: sequential? function
            if first.value == 'sequential?':
                # Check if something is a list or vector
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    return FALSE
                return MalBoolean(isinstance(args[0], (MalList, MalVector)))
            ...
            if first.value == 'fn?':
                ...
                if not args:
                    return FALSE
                return MalBoolean(isinstance(args[0], MalFunction))

            # Step 7: cons function
            if first.value == 'cons':
                # (cons element seq) - add element to front of list/vector
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if len(args) != 2:
                    raise Exception("cons requires 2 arguments")
                element, seq = args[0], args[1]
                if isinstance(seq, MalList):
                    return MalList([element] + seq.items)
                elif isinstance(seq, MalVector):
                    return MalList([element] + seq.items)
                else:
                    raise Exception("cons: second argument must be a list or vector")

            # Step 7: concat function
            if first.value == 'concat':
                # (concat seq...) - concatenate lists/vectors
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                result = []
                for arg in args:
                    if isinstance(arg, MalList):
                        result.extend(arg.items)
                    elif isinstance(arg, MalVector):
                        result.extend(arg.items)
                    else:
                        raise Exception("concat: all arguments must be lists or vectors")
                return MalList(result)

            # Step 7: vec function
            if first.value == 'vec':
                # (vec seq) - convert list to vector (or return vector as-is)
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    raise Exception("vec requires an argument")
                seq = args[0]
                if isinstance(seq, MalList):
                    return MalVector(seq.items[:])
                elif isinstance(seq, MalVector):
                    return seq
                else:
                    raise Exception("vec: argument must be a list or vector")
            ...
            # Step 8: nth function
            if first.value == 'nth':
                # (nth seq index) - get element at index
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if len(args) != 2:
                    raise Exception("nth requires 2 arguments")
                seq, idx = args[0], args[1]
                if not isinstance(idx, MalNumber):
                    raise Exception("nth: index must be a number")
                if not isinstance(seq, (MalList, MalVector)):
                    raise Exception("nth: first argument must be a list or vector")
                if 0 <= idx.value < len(seq.items):
                    return seq.items[idx.value]
                else:
                    raise Exception(f"nth: index {idx.value} out of range")

            # Step 8: first function
            if first.value == 'first':
                ...
                if not args:
                    return NIL
                seq = args[0]
                if isinstance(seq, MalNil):
                    return NIL
                if isinstance(seq, (MalList, MalVector)):
                    if seq.items:
                        return seq.items[0]
                    else:
                        return NIL
                else:
                    raise Exception("first: argument must be a list or vector")

            # Step 8: rest function
            if first.value == 'rest':
                # (rest seq) - get all elements except first
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    return MalList([])
                seq = args[0]
                ...
                if isinstance(seq, (MalList, MalVector)):
                    if seq.items:
                        return MalList(seq.items[1:])
                    else:
                        return MalList([])
                else:
                    raise Exception("rest: argument must be a list or vector")

            # Step 9: apply function
            if first.value == 'apply':
                # (apply f args...) - apply function to list of arguments
                # Last argument must be a list
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    raise Exception("apply requires at least one argument")

                func = args[0]
                apply_args = args[1:-1]  # Arguments before the last list
                last_arg = args[-1] if len(args) > 1 else None

                if last_arg is None:
                    # No list argument, just call function with no args
                    all_args = []
                elif isinstance(last_arg, MalList):
                    all_args = apply_args + last_arg.items
                elif isinstance(last_arg, MalVector):
                    all_args = apply_args + last_arg.items
                elif isinstance(last_arg, MalNil):
                    all_args = apply_args
                else:
                ...
                if isinstance(func, MalMacro):
                    ...
                    if variadic_index is not None:
                        ...
                        for i, param in enumerate(regular_params):
                            if i < len(all_args):
                                fn_env.set(param.value, all_args[i])
                            else:
                                fn_env.set(param.value, NIL)
                    ...
                    for body_expr in body_items[:-1]:
                        EVAL(body_expr, fn_env)
                    return EVAL(body_items[-1], fn_env)
                elif callable(func):
                    # Built-in function
                    return func(*all_args)
                else:
                    raise Exception("apply: first argument must be a function")
            ...
            # Step 8: macroexpand function
            if first.value == 'macroexpand':
                # Expand macros repeatedly until no more macros
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    raise Exception("macroexpand requires an argument")
                return macroexpand(args[0], env)

            # Step 8: macroexpand-1 function
            if first.value == 'macroexpand-1':
                # Expand macro once
                args = [EVAL(arg, env) for arg in ast.items[1:]]
                if not args:
                    raise Exception("macroexpand-1 requires an argument")
                return macroexpand(args[0], env, once=True)
        ...
        # First, evaluate the first element to see if it's a function/macro
        first = ast.items[0]
        func = None
        if isinstance(first, MalSymbol):
            try:
                ...
                pass  # func stays None

        # Step 8: Check if first element is a macro (before evaluating arguments)
        if func is not None and isinstance(func, MalMacro):
            ...
            # The last expression is the expanded form
            expanded = EVAL(body_items[-1], macro_env)

            # Continue evaluation with the expanded form
            ast = expanded
            continue

        # For non-macros, evaluate all elements and apply
...
def main():
    ...
    # Step 9: Add throw function
    def throw_fn(value):
        raise MalThrownException(value)

    # Step 9: Add nil? function
    def nil_q_fn(x):
        return MalBoolean(isinstance(x, MalNil))

    # Step 9: Add true? function
    def true_q_fn(x):
        return MalBoolean(isinstance(x, MalBoolean) and x.value)

    # Step 9: Add false? function
    def false_q_fn(x):
    ...
    # Step A: Add time-ms function
    import time
    def time_ms_fn():
        return MalNumber(int(time.time() * 1000))

    # Step A: Add number? function
    def number_q_fn(x):
        return MalBoolean(isinstance(x, MalNumber))

    # Step A: Add readline function
    def readline_fn(prompt):
        if isinstance(prompt, MalString):
    ...
    # Step A: Add string? function
    def string_q_fn(x):
        return MalBoolean(isinstance(x, MalString))

    # Step A: Add conj function
    def conj_fn(*args):
        if not args:
            raise Exception("conj requires at least one argument")
        seq = args[0]
        forms = args[1:]
        if isinstance(seq, MalVector):
            # For vectors, just append elements
            return MalVector(seq.items + list(forms))
        elif isinstance(seq, MalList):
            # For lists, prepend elements in reverse order
            return MalList(list(reversed(forms)) + seq.items)
        else:
            raise Exception("conj: first argument must be a list or vector")
    ...
    repl_env.set('*', mul_fn)
    repl_env.set('/', div_fn)
    repl_env.set('throw', throw_fn)
    repl_env.set('nil?', nil_q_fn)
    ...
    repl_env.set('string?', string_q_fn)
    repl_env.set('conj', conj_fn)

    # Add *ARGV* as an empty list (for tests)
    repl_env.set('*ARGV*', MalList([]))

    # Step A: Add *host-language* variable
    repl_env.set('*host-language*', MalString("python"))

    # Step 8: Define not function (for cond macro and other uses)
    not_fn_code = READ("(fn* (a) (if a false true))")
    not_fn = EVAL(not_fn_code, repl_env)
    repl_env.set('not', not_fn)

    # Step 8: Define cond macro using rep (like reference)
    ...
    # Interactive mode
    print("Mal Step A - Mal Self-Host REPL")
